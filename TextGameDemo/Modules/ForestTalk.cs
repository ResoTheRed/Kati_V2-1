...
namespace TextGameDemo.Modules {
    public class ForestTalk : Module {
        ...
        private string[] topics = { Talk };
        private ForestTalkParser parse;
        ...
        public ForestTalk(string path, GameModel model) : base(JsonToolkit.FOREST_TALK, path) {
            ForestTalkParser parse = new ForestTalkParser(Ctrl);
            parse.SetGameRules(model.GameData);
            parse.SetLocationRules(model.Player);
            Ctrl.Parser = parse;
            Ctrl.Game = model.GameData;
            ForestTalkResponse res = new ForestTalkResponse();
        ...
        public void SetCurrentCharacter(string character) {
            ...
            ForestTalkParser v = (ForestTalkParser)Ctrl.Parser;
        ...
        public string SetType(DialoguePackage pack, string topic) {
            ...
            if (pack != null && pack.Type == Kati.Constants.RESPONSE) {
                ...
                if (GameTools.Tools().Next(10) > 7 && topic.Equals(Talk)) {
    ...
    class ForestTalkParser : Parser {

        ForestTalkPersonalRules personal;
        ForestTalkGameRules gRules;
        ForestTalkHistoryRules hRules;
        ForestTalkLocationRules local;
        ForestTalkLeadToRules leadTo;
        Game.Characters.Character npc;
        GameModel model;

        public ForestTalkPersonalRules Personal { get => personal; set => personal = value; }
        public Character Npc { get => npc; set => npc = value; }
        public ForestTalkGameRules GRules { get => gRules; set => gRules = value; }
        //public GameModel Model { get => model; set => model = value; }
        public ForestTalkHistoryRules HRules { get => hRules; set => hRules = value; }
        internal ForestTalkLeadToRules LeadTo { get => leadTo; set => leadTo = value; }

        public ForestTalkParser(Controller ctrl) : base(ctrl) {
            Personal = new ForestTalkPersonalRules(ctrl); ;
            LeadTo = new ForestTalkLeadToRules();
            //GRules = new GameRules(Model);
        }

        public void SetHistoryRules(History h) {
            HRules = new ForestTalkHistoryRules(h);
        }

        public void SetGameRules(Game.GameData game) {
            GRules = new ForestTalkGameRules(game);
        }
        public void SetLocationRules(Player local) {
            this.local = new ForestTalkLocationRules(local);
        }
    ...
    class ForestTalkResponse : Response {
    ...
    class ForestTalkPersonalRules {

        Controller ctrl;

        public ForestTalkPersonalRules(Controller ctrl) {
            this.ctrl = ctrl;
        }
    ...
    class ForestTalkGameRules {

        Game.GameData game;

        public ForestTalkGameRules(Game.GameData game) {
            this.game = game;
        }
    ...
    class ForestTalkLocationRules {

        Player local;

        public ForestTalkLocationRules(Player local) {
            this.local = local;
        }
    ...
    class ForestTalkHistoryRules {

        private History h;
        private Dictionary<string, Dictionary<string, bool>> playOnce;

        public ForestTalkHistoryRules(History h) {
            this.h = h;
    ...
    class ForestTalkLeadToRules {

        public ForestTalkLeadToRules() { }
    ...
    class ForestTalkBranchDecision : BranchDecision {



        public ForestTalkBranchDecision(Controller ctrl) : base(ctrl) { }
