...
namespace TextGameDemo.Modules {
    ...
    public class YoungLove : Module {
        ...
        const string PUPPY_LOVE = "puppyLove";
        const string IS_IT_LOVE = "IsItLove";
        const string SOUL_MATES = "SoulMates";

        private string[] topics = { PUPPY_LOVE, IS_IT_LOVE, SOUL_MATES};
        private YoungLoveParser parse;
        ...
        public YoungLove(string path, GameModel model) : base(JsonToolkit.AROUND_TOWN, path) {
            YoungLoveParser parse = new YoungLoveParser(Ctrl);
            parse.SetGameRules(model.GameData);
            parse.SetLocationRules(model.Player);
            Ctrl.Parser = parse;
            Ctrl.Game = model.GameData;
            YoungLoveResponse res = new YoungLoveResponse();
        ...
        public void SetCurrentCharacter(string character) {
            ...
            YoungLoveParser v = (YoungLoveParser)Ctrl.Parser;
        ...
        public string SetTopic() {
            ...
            if (stats[Constants.ROMANCE] < 250) {
                topic = PUPPY_LOVE;
            } else if (stats[Constants.ROMANCE] < 600) {
                topic = IS_IT_LOVE;
            } else {
                topic = SOUL_MATES;
            }
        ...
        public string SetType(DialoguePackage pack, string topic) {
            ...
            if (pack != null && pack.Type == Constants.RESPONSE) {
                ...
                if ( value > 7 && topic.Equals(PUPPY_LOVE) || value > 7 && topic.Equals(IS_IT_LOVE)) {
    ...
    class YoungLoveParser : Parser {

        YoungLovePersonalRules personal;
        YoungLoveGameRules gRules;
        YoungLoveHistoryRules hRules;
        YoungLoveLocationRules local;
        YoungLoveLeadToRules leadTo;
        Game.Characters.Character npc;
        GameModel model;

        public YoungLovePersonalRules Personal { get => personal; set => personal = value; }
        public Character Npc { get => npc; set => npc = value; }
        public YoungLoveGameRules GRules { get => gRules; set => gRules = value; }
        public GameModel Model { get => model; set => model = value; }
        public YoungLoveHistoryRules HRules { get => hRules; set => hRules = value; }
        internal YoungLoveLeadToRules LeadTo { get => leadTo; set => leadTo = value; }

        public YoungLoveParser(Controller ctrl) : base(ctrl) {
            Personal = new YoungLovePersonalRules(ctrl);
            LeadTo = new YoungLoveLeadToRules();
        }

        public void SetHistoryRules(History h) {
            HRules = new YoungLoveHistoryRules(h);
        }

        public void SetGameRules(Game.GameData game) {
            GRules = new YoungLoveGameRules(game);
        }
        public void SetLocationRules(Player local) {
            this.local = new YoungLoveLocationRules(local);
        }
    ...
    class YoungLoveResponse : Response {
    ...
    class YoungLovePersonalRules {

        Controller ctrl;

        public YoungLovePersonalRules(Controller ctrl) {
            this.ctrl = ctrl;
        }
    ...
    class YoungLoveGameRules {

        Game.GameData game;

        public YoungLoveGameRules(Game.GameData game) {
            this.game = game;
        }
    ...
    class YoungLoveLocationRules {

        Player local;

        public YoungLoveLocationRules(Player local) {
            this.local = local;
        }
    ...
    class YoungLoveHistoryRules {

        private History h;
        private Dictionary<string, Dictionary<string, bool>> playOnce;

        public YoungLoveHistoryRules(History h) {
            this.h = h;
    ...
    class YoungLoveLeadToRules {

        public YoungLoveLeadToRules() { }
    ...
    class YoungLoveBranchDecision : BranchDecision {



        public YoungLoveBranchDecision(Controller ctrl) : base(ctrl) { }
